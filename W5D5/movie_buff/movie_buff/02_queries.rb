...
def vanity_projects
  ...
  # Movie.select(:id, :title, :director_id).joins(:actors).where("actor_id = director_id")
...
def most_supportive
  ...
  Casting.select(:actor_id, "COUNT(*)").where.not(ord: 1).group(:actor_id).order("COUNT(*) DESC").limit(2).as_json
end
